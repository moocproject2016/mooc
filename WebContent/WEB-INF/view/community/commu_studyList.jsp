...
<div class="tableWrap">
	...
	<table class="table" align="center">
		<tr valign="middle" class="tableHead">
			<td colspan="5">
				<h4>${name}채널에개설된 SturdyRoom 목록입니다.</h4>
				<form
					action="/mooc/study/studylist.mooc?sub_ctg_code=${sub_ctg_code}"
					method="post">
		...
		<c:forEach var="stglist" items="${list}" varStatus="i">
			<tr height="30">
				<td align="center"
					onClick="purposeDisplay('purpose_${stglist.stg_code}')"
					style='cursor: pointer;'>${stglist.stg_name}</td>
				<td align="center"><input type="text" name="stg_people"
					value="${stglist.stg_count}" size=1
					style="text-align: right; border: 0px; background-color: transparent;"
					readonly /> /<input type="text" name="stg_limit"
					value="${stglist.stg_limit}" size=2
					style="border: 0px; background-color: transparent;" readonly /></td>
				<td align="center">
				...
				<td align="center">${stglist.u_name}</td>
				<td align="center"><fmt:formatDate
						value="${stglist.stg_regdate}" pattern="yyyy-MM-dd" /></td>
			</tr>
		...
		<c:if test="${list.size() == 0}">
			<tr>
				<td colspan="4">개설된 스터디룸이 없습니다.</td>
			<tr>
		</c:if>
	</table>
					...
					<nav>
						<ul class="pagination">
							...
							<c:if test="${startPage > 10}">
								<li><a
									href="/mooc/study/studylist.mooc?sub_ctg_code=${sub_ctg_code}&pageNum=${startPage - 10 }"
							...
							<c:forEach var="i" begin="${startPage}" end="${endPage}">
								<c:if test="${currentPage==i}">
									<li class="active"><a
										href="/mooc/study/studylist.mooc?sub_ctg_code=${sub_ctg_code}&pageNum=${i}">${i}</a></li>
								</c:if>
								<c:if test="${currentPage!=i}">
									<li><a
										href="/mooc/study/studylist.mooc?sub_ctg_code=${sub_ctg_code}&pageNum=${i}">${i}</a></li>
