...
<script>
	...
	function select() {
		selectMain = document.getElementById("main_select").value;
		window.location = '/mooc/teacher/reviewList.mooc?main_lec_code='
				+ selectMain;
	}
	...
	<table class="table" align="center">
		...
		<tbody>
			<c:forEach var="lec_review" items="${member}" varStatus="i">
				<tr>
					<td>${lec_review.main_lec_code}<input type="hidden"
						name="main_lec_code" value="${lec_review.main_lec_code}" /><input
						type="hidden" name="index" value="${index}" id="${index}" /></td>
					<td onclick="displaySwitch('${i.count}re');"><b>${lec_review.lec_r_subject}</b>
					</td>
					<td>${lec_review.u_id}</td>
					<td>평점 : ${lec_review.lec_r_score}</td>
					<td><c:if
							test="${lec_review.lec_r_score<=5 && lec_review.lec_r_score>4}">★★★★★</c:if>
						<c:if
							test="${lec_review.lec_r_score<=4 && lec_review.lec_r_score>3}">★★★★☆</c:if>
						<c:if
							test="${lec_review.lec_r_score<=3 && lec_review.lec_r_score>2}">★★★☆☆</c:if>
						<c:if
							test="${lec_review.lec_r_score<=2 && lec_review.lec_r_score>1}">★★☆☆☆</c:if>
						<c:if
							test="${lec_review.lec_r_score<=1 && lec_review.lec_r_score>0}">★☆☆☆☆</c:if>
					</td>
				...
				<c:if test="${lec_review.lec_r_content!=null}">
					<tr>
						<td colspan="7" align="left">
							<div id="content${i.count}re"
								style="display: none; height: auto;">
								후기
								내용&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${lec_review.lec_r_content}<br />
								<br />
								<hr>
							</div>
						</td>
					...
					<nav>
						<ul class="pagination">
							...
							<c:if test="${startPage > 10}">
								<li><a
									href="reviewList.mooc?pageNum=${startPage - 10 }&main_lec_code=${main_lec_code}"
							...
							<c:forEach var="i" begin="${startPage}" end="${endPage}">
								<c:if test="${currentPage==i}">
									<li class="active"><a
										href="reviewList.mooc?pageNum=${i}&main_lec_code=${main_lec_code}">${i}</a></li>
								</c:if>
								<c:if test="${currentPage!=i}">
									<li><a
										href="reviewList.mooc?pageNum=${i}&main_lec_code=${main_lec_code}">${i}</a></li>
