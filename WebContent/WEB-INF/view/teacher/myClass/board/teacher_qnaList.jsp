...
<head>
	...
	<script>			
		...
		function select(){
			selectMain=document.getElementById("main_select").value;
			window.location='/mooc/teacher/qnaList.mooc?main_lec_code='+selectMain;
		}
					...
					<tr>
						...
						<td colspan="7" align="left">
							<div id="content${i.count}" style="display:none; height:auto;">
								...
								<hr>
								<c:if test="${u_type==1}">
								<form name="contentUpdate" action="/mooc/teacher/view_ReContent3.mooc">
				...
				<td height="50" align="center">
					<c:if test="${count > 0}">
						...
						<c:if test="${startPage > 10}">
							<a href="/mooc/teacher/qnaList.mooc?main_lec_code=${main_lec_code }&pageNum=${startPage - 10 }">[이전]</a>
						</c:if>
				
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<a href="/mooc/teacher/qnaList.mooc?main_lec_code=${main_lec_code }&pageNum=${i}">[${i}]</a>
						</c:forEach>
					
						<c:if test="${endPage < pageCount}">
							<a href="/mooc/teacher/qnaList.mooc?main_lec_code=${main_lec_code }&pageNum=${startPage + 10}">[다음]</a>
						</c:if>
					</c:if>
				</td>
			</tr>
