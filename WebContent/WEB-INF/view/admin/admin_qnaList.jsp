	...
	<form name="updateform" method="post" action="/mooc/admin/qnaWriteForm.mooc">
		...
		<table class="table" align="center">
			<tr>
				<td colspan="6">
					<h3>문의관리</h3>
					<select name="search" id="selected">
						<option>제목</option>
						<option >아이디</option>
					</select>
					<input type="text" name="select" id="search">
					<input type="button" value="검색" onclick="go();">
				</td>
			</tr>
				<tr class="theadtop">
					...
					<tr>
						...
						<td colspan="7" align="left">
							<div id="content${i.count}" style="display:none; height:auto;">
								...
								<hr>
								<form name="contentUpdate" action="/mooc/admin/qnaWriteForm.mooc">
			...
			<td height="50" align="center">
				<c:if test="${count > 0}">
					...
					<c:if test="${startPage > 10}">
						<a href="/mooc/admin/qnaList.mooc?pageNum=${startPage - 10 }">[이전]</a>
					</c:if>
			
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<a href="/mooc/admin/qnaList.mooc?pageNum=${i}">[${i}]</a>
					</c:forEach>
				
					<c:if test="${endPage < pageCount}">
						<a href="/mooc/admin/qnaList.mooc?pageNum=${startPage + 10}">[다음]</a>
					</c:if>
				</c:if>
			</td>
		</tr>
