...
<ul class="nav nav-pills">
	...
	<li role="presentation" class="active"><a href="myQuestion.mooc">관리자
			문의 / 답변</a></li>
...
<form name="updateform">
	...
	<table class="table" align="center">
		<tr>
			<td>
				<h1>
					내가 쓴 게시글<small>&nbsp;&nbsp;&nbsp;&nbsp;관리자 문의 / 답변</small>
				</h1>
			...
			<c:forEach var="question" items="${question}" varStatus="i">
				<tr>
					<td align="center"><input type="checkbox" name="checkbox"
						value="${question.q_num}" /></td>
					<td align="center">${question.q_num}<input type="hidden"
						name="q_num" value="${question.q_num}" /><input type="hidden"
						name="index" value="${index}" id="${index}" />
					</td>
					<td align="center" onclick="displaySwitch('${i.count}');"><b><c:set
								var="date" value="${fn:split(question.q_subject, ' ')}" />${date[0]}</b>
					</td>
					<td align="center"><c:set var="date"
							value="${fn:split(question.q_regdate, ' ')}" /> ${date[0]}</td>
				</tr>
...
<table class="table" align="center">
	<tr>
		<td>
			<form action="/mooc/user/myQuestion.mooc" method="post">
				<select name="search">
				...
				<nav>
					<ul class="pagination">
						...
						<c:if test="${startPage > 10}">
							<li><a href="myQuestion.mooc?pageNum=${startPage - 10 }"
