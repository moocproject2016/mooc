...
<ul class="nav nav-pills">
	...
	<li role="presentation" class="active"><a
		href="myLec_questionList.mooc">강의 질문/ 답변</a></li>
</ul>
...
<form name="updateform">
	...
	<table class="table" align="center">
		...
		<tbody>
			<c:forEach var="lecture_question" items="${lecture_question}"
				varStatus="i">
				<tr>
					<td align="center"><input type="checkbox" name="checkbox"
						value="${lecture_question.lec_q_num}" /></td>
					<td align="center">${lecture_question.lec_q_num}<input
						type="hidden" name="lec_q_num"
						value="${lecture_question.lec_q_num}" /> <input type="hidden"
					...
					<td align="center"><c:set var="date"
							value="${fn:split(lecture_question.lec_q_regdate, ' ')}" />
						${date[0]}</td>
				</tr>
				<c:if test="${lecture_question.lec_q_content!=null}">
					<tr>
						<td colspan="7" align="left">
							<div id="content${i.count}re2"
								style="display: none; height: auto;">
								문의 내용
								&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${lecture_question.lec_q_content}<br />
								<br />
								<hr>
							</div>
						</td>
...
<table class="table" align="center">
	<tr>
		<td>
			...
			<form action="/mooc/user/myLec_questionList.mooc" method="post">
				<select name="search">
				...
				<nav>
					<ul class="pagination">
						...
						<c:if test="${startPage > 10}">
							<li><a
								href="myLec_questionList.mooc?pageNum=${startPage - 10 }"
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<c:if test="${currentPage==i}">
								<li class="active"><a
									href="myLec_questionList.mooc?pageNum=${i}">${i}</a></li>
							</c:if>
							<c:if test="${currentPage!=i}">
