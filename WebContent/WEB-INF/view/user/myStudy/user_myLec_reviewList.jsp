...
<ul class="nav nav-pills">
	...
	<li role="presentation" class="active"><a
		href="myLecture_review.mooc">강의 후기</a></li>
			...
			<c:forEach var="lec_review" items="${lec_review}" varStatus="i">
				<tr>
					<td align="center"><input type="checkbox" name="checkbox"
						value="${lec_review.main_lec_code}" /></td>
					<td align="center">${lec_review.main_lec_code}<input
						type="hidden" name="main_lec_code"
						value="${lec_review.main_lec_code}" /><input type="hidden"
						name="index" value="${index}" id="${index}" /></td>
					<td align="center">강의 후기</td>
					<td align="center" onclick="displaySwitch('${i.count}re');"><b>${lec_review.lec_r_subject}</b>
					</td>
					<td align="center"><c:set var="date"
							value="${fn:split(lec_review.lec_r_regdate, ' ')}" /> ${date[0]}
					</td>
				</tr>
...
<table class="table" align="center">
	<tr>
		<td>
			<form action="/mooc/user/myLecture_review.mooc" method="post">
				<select name="search">
		...
		<td align="right"><a onclick="deleteNums()">선택삭제</a></td>
	</tr>
				...
				<nav>
					<ul class="pagination">
						...
						<c:if test="${startPage > 10}">
							<li><a
								href="myLecture_review.mooc?pageNum=${startPage - 10 }"
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<c:if test="${currentPage==i}">
								<li class="active"><a
									href="myLecture_review.mooc?pageNum=${i}">${i}</a></li>
							</c:if>
							<c:if test="${currentPage!=i}">
